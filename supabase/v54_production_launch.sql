-- PERPHOOD V54 — real Robinhood Chain token metadata and confirmed launch registry.
...
create table if not exists public.perphood_v54_launches (
...
create index if not exists perphood_v54_launches_chain_block_idx
  on public.perphood_v54_launches (chain_id, block_number desc);
create index if not exists perphood_v54_launches_creator_idx
  on public.perphood_v54_launches (creator_address, block_number desc);
create index if not exists perphood_v54_launches_symbol_idx
  on public.perphood_v54_launches (symbol, block_number desc);

alter table public.perphood_v54_launches enable row level security;

drop policy if exists "Public can read confirmed V54 launches" on public.perphood_v54_launches;
create policy "Public can read confirmed V54 launches"
  on public.perphood_v54_launches
  for select
...
-- Public token images and metadata are readable because token-media is a public bucket.
