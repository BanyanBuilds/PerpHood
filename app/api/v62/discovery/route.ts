...
export async function GET(request: Request) {
  if (!isV54LaunchStorageConfigured()) return response({ ok: true, configured: false, protocol: "LEVERAGE X V62", markets: [] });
  try {
    ...
    return response({ ok: true, configured: true, protocol: "LEVERAGE X V62", markets });
  } catch (error) {
