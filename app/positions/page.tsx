...
export default function PositionsPage() {
  ...
  const shareOpen = async (position: Position, token: Token) => {
    ...
    try { await navigator.clipboard.writeText(`${token.symbol} ${position.leverage}× ${position.direction.toUpperCase()} · ${roi >= 0 ? "+" : ""}${roi.toFixed(1)}% P&L on LEVERAGE X`); } catch {}
