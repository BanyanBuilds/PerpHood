...
export const V45_FACTORY_EVENT = "MarketCreated(address,address,address,uint256,uint256,bytes32)";

export type V45LaunchInput = {
  name: string;
  symbol: string;
  description: string;
...
export function encodeV45CreateMarket(
  name: string,
  symbol: string,
  metadataHash: Hex,
  targetUsdWad: bigint,
...
export function v45MetadataHash(input: V45LaunchInput) {
...
export function parseV45MarketCreated(receipt: TransactionReceipt): Pick<V45LaunchReceipt, "marketAddress" | "tokenAddress" | "creatorAddress" | "blockNumber"> {
  const topic = eventTopic(V45_FACTORY_EVENT).toLowerCase();
...
export async function launchV45Market(
  input: V45LaunchInput,
  factoryAddress: string,
  provider: Eip1193Provider | null = injectedProvider(),
  rpcUrl = process.env.NEXT_PUBLIC_LOCAL_CHAIN_RPC ?? DEFAULT_LOCAL_RPC,
): Promise<V45LaunchReceipt> {
  if (!provider) throw new Error("No injected EVM wallet was found.");
  if (!/^0x[0-9a-fA-F]{40}$/.test(factoryAddress)) throw new Error("The V45 local factory address is invalid.");
  ...
  const data = encodeV45CreateMarket(
    input.name,
    input.symbol,
    v45MetadataHash(input),
  ...
  if (receipt.status === "0x0") throw new Error("The V45 local launch transaction reverted.");
  return { account, transactionHash, ...parseV45MarketCreated(receipt) };
}

export async function readV45FactoryMarketCount(
  factoryAddress: string,
  rpcUrl = process.env.NEXT_PUBLIC_LOCAL_CHAIN_RPC ?? DEFAULT_LOCAL_RPC,
) {
