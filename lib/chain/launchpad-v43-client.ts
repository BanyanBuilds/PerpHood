...
export const V43_FACTORY_EVENT = "MarketCreated(address,address,address,uint256,uint256,bytes32)";

export type V43LaunchInput = {
  name: string;
  symbol: string;
  description: string;
...
export function encodeV43CreateMarket(
  name: string,
  symbol: string,
  metadataHash: Hex,
  targetUsdWad: bigint,
...
export function v43MetadataHash(input: V43LaunchInput) {
...
export function parseV43MarketCreated(receipt: TransactionReceipt): Pick<V43LaunchReceipt, "marketAddress" | "tokenAddress" | "creatorAddress" | "blockNumber"> {
  const topic = eventTopic(V43_FACTORY_EVENT).toLowerCase();
...
export async function launchV43Market(
  input: V43LaunchInput,
  factoryAddress: string,
  provider: Eip1193Provider | null = injectedProvider(),
  rpcUrl = process.env.NEXT_PUBLIC_LOCAL_CHAIN_RPC ?? DEFAULT_LOCAL_RPC,
): Promise<V43LaunchReceipt> {
  if (!provider) throw new Error("No injected EVM wallet was found.");
  if (!/^0x[0-9a-fA-F]{40}$/.test(factoryAddress)) throw new Error("The V43 local factory address is invalid.");
  ...
  const data = encodeV43CreateMarket(
    input.name,
    input.symbol,
    v43MetadataHash(input),
  ...
  if (receipt.status === "0x0") throw new Error("The V43 local launch transaction reverted.");
  return { account, transactionHash, ...parseV43MarketCreated(receipt) };
}

export async function readV43FactoryMarketCount(
  factoryAddress: string,
  rpcUrl = process.env.NEXT_PUBLIC_LOCAL_CHAIN_RPC ?? DEFAULT_LOCAL_RPC,
) {
