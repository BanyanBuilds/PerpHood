/** Local V23 custody + full-action session-key ABI. This is an unaudited development boundary. */
export const LOCAL_BATTLE_POOL_V23_ABI = [
  ...
  "function commitSingleAccountFrame(uint64 expectedSequence,bytes32 expectedPreviousStateHash,(bytes32 marketId,uint8 action,uint256 marginalPriceWad,uint256 marketCapWad,uint256 reservedWethWad,uint256 openInterestLongWad,uint256 openInterestShortWad,bytes32 positionsRoot,bytes32 balancesRoot,bytes32 intentHash) frame,address account,int256 accountWethDeltaWad,int256 accountTokenDelta,int256 poolWethDeltaWad,int256 poolTokenDelta) returns (bytes32 nextStateHash)",
