...
import {LeverageXLaunchFactoryV55, LeverageXSpotMarketV55, LeverageXTokenV55} from "../src/LeverageXLaunchFactoryV55.sol";
...
contract LeverageXLaunchFactoryV55Test {
    VmV55 internal constant vm = VmV55(address(uint160(uint256(keccak256("hevm cheat code")))));
    address internal creator = address(0xC0FFEE);
    address internal trader = address(0xB0B);
    LeverageXLaunchFactoryV55 internal factory;

    function setUp() public {
        factory = new LeverageXLaunchFactoryV55(address(this));
        vm.deal(creator, 10 ether);
        vm.deal(trader, 10 ether);
    }

    function _launch() internal returns (LeverageXSpotMarketV55 market, LeverageXTokenV55 token) {
        vm.prank(creator);
    ...
    function testCreatesRealFixedSupplyTokenAndRegistersMarket() public {
        (LeverageXSpotMarketV55 market, LeverageXTokenV55 token) = _launch();
    ...
    function testCreatorIsPermanentlyMarkedPerpsRestricted() public {
        (LeverageXSpotMarketV55 market,) = _launch();
    ...
    function testZeroGenesisBuyAndEmptyMetadataRevert() public {
        ...
        vm.expectRevert(LeverageXLaunchFactoryV55.InvalidMetadata.selector);
        vm.prank(creator);
    ...
    function testOnlyFactoryOwnerCanPauseMarket() public {
        (LeverageXSpotMarketV55 market,) = _launch();
        vm.expectRevert(LeverageXLaunchFactoryV55.OnlyOwner.selector);
        vm.prank(trader);
        factory.setMarketPaused(market, true);
        factory.setMarketPaused(market, true);
        require(market.paused(), "NOT_PAUSED");
        vm.expectRevert(LeverageXSpotMarketV55.MarketPaused.selector);
        vm.prank(trader);
