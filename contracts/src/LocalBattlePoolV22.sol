...
/// @title BattleTokenV22
/// @notice Minimal one-billion-supply token used by the V22 local-chain BattlePool.
...
/// @title LocalBattlePoolV22
/// @notice V22 local-chain custody and ordered settlement spine for PERPHOOD.
/// @dev The deterministic TypeScript engine remains the pricing and liquidation oracle in V22.
...
contract LocalBattlePoolV22 {
    ...
    BattleTokenV22 public immutable token;
    ...
    constructor(address owner_, address sequencer_, bytes32 marketId_, string memory tokenName_, string memory tokenSymbol_) {
        ...
        marketId = marketId_;
        token = new BattleTokenV22(tokenName_, tokenSymbol_, TOTAL_TOKEN_SUPPLY, address(this));
    ...
    /// @notice Authorizes one browser-held session key for gas-sponsored trading intents.
    /// @dev The main wallet calls this once. The V22 sequencer verifies P-256 signatures off-chain,
    ///      while this contract independently enforces owner, expiry, nonce, action scope, and size.
    function authorizeSession(
        ...
        if (sessionId == bytes32(0) || publicKeyHash == bytes32(0)) revert InvalidSession();
        if (validUntil <= block.timestamp || maxNotionalWad == 0 || actionBitmap == 0) revert InvalidSession();
        ...
        _sessions[sessionId] = SessionAuthorization({
            owner: msg.sender,
            publicKeyHash: publicKeyHash,
            validUntil: validUntil,
            nextNonce: preservedNonce,
            maxNotionalWad: maxNotionalWad,
            actionBitmap: actionBitmap,
            active: true
        });
    ...
    /// @notice Gas-sponsored settlement path for a previously wallet-authorized session.
    /// @dev Signature verification happens in the V22 relay; this contract constrains what that
    ///      relay may settle and makes every intent nonce single-use.
