...
interface ILeverageXFactorySafetyV56 {
    function globalTradingPaused() external view returns (bool);
}

/// @title LeverageXTokenV56
...
/// @title LeverageXSpotMarketV56
/// @notice Native-ETH bonding-curve market with mainnet canary safety controls.
/// @dev V56 remains Spot-only. Long/Short stays disabled until the BattlePool receives a separate
///      audited deployment and explicit activation transaction.
contract LeverageXSpotMarketV56 {
    ...
    address public immutable factory;
    address public immutable creator;
    LeverageXTokenV56 public immutable token;
    bytes32 public immutable metadataHash;
        ...
        )
    {
        ...
        trades = tradeCount;
        isPaused = paused;
        isGloballyPaused = ILeverageXFactorySafetyV56(factory).globalTradingPaused();
        buyCapWei = maxBuyWei;
    ...
    function _requireTradingEnabled() internal view {
        if (paused || ILeverageXFactorySafetyV56(factory).globalTradingPaused()) revert MarketPaused();
    }
...
contract LeverageXLaunchFactoryV56 {
    ...
    LeverageXSpotMarketV56[] public markets;
    ...
    function createMarket(
        ...
        bytes32 metadataHash,
        uint256 migrationTargetUsdWad
    ) external payable nonReentrant returns (LeverageXSpotMarketV56 market, LeverageXTokenV56 token) {
        if (launchMode == LaunchMode.Closed) revert LaunchClosed();
        if (launchMode == LaunchMode.Allowlist && !canaryCreator[msg.sender]) revert CreatorNotAllowed();
        ...
        market = new LeverageXSpotMarketV56{value: msg.value}(
            msg.sender,
            name,
            symbol,
            metadataURI,
