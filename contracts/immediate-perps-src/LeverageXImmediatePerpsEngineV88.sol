...
contract LeverageXImmediatePerpsEngineV88 {
    ...
    address public marketBootstrapper;
    ...
    event MarketBootstrapperSet(address indexed bootstrapper);
    event MintedMarketRiskEnabled(address indexed token, RiskConfig config);
    ...
    function setMarketBootstrapper(address bootstrapper) external onlyOwner {
        if (bootstrapper == address(0)) revert InvalidAddress();
        marketBootstrapper = bootstrapper;
        emit MarketBootstrapperSet(bootstrapper);
    }

    /// @notice Atomically enables conservative risk limits during token mint.
    function bootstrapMintedMarket(address token, RiskConfig calldata config) external {
        if (msg.sender != marketBootstrapper) revert OnlyMarketBootstrapper();
        if (riskConfig[token].enabled) revert MarketAlreadyConfigured();
        if (token == address(0) || config.maintenanceMarginBps == 0 || config.maintenanceMarginBps >= BPS) revert InvalidRiskConfig();
        if (config.openFeeBps > 500 || config.closeFeeBps > 500 || config.maxOracleAgeSeconds == 0 || config.maxPositionNotionalWei == 0 || !config.enabled) revert InvalidRiskConfig();
        riskConfig[token] = config;
        emit RiskConfigSet(token, config);
        emit MintedMarketRiskEnabled(token, config);
    }
