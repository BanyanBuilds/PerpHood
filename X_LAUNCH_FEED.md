...
Leverage X V33 removes the generic synthetic X tracker and replaces it with a launch-specific workflow:

```text
Official X post
  → Leverage X launch rule matches
...
## Why Leverage X does not import Axiom or Terminal/Padre

Their documented X tools are product features, not published feed-export or embed APIs. Leverage X can import a user's account list as JSON, CSV, or text, but does not scrape a competitor's private UI, credentials, or data stream.
...
It manages official filtered-stream rules, receives posts as X publishes them, and sends normalized posts into the protected Leverage X ingest endpoint.

Required:

```env
X_BEARER_TOKEN=
LEVERAGE X_X_INGEST_URL=http://localhost:3000/api/x-launch-feed/ingest
...
- Leverage X never invents social posts when the API is unavailable.
