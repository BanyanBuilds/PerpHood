...
const deploymentPath = join(root, "public", "local-chain", "v43-deployment.json");
...
type DeploymentManifest = {
  version: "v43-unified-battlepool";
  chainId: number;
  rpcUrl: string;
  owner: string;
  sequencer: string;
...
async function compileFactory() {
  try {
    ...
    throw new Error("Foundry is required for the V43 bootstrap. Install Foundry, then retry npm run chain:v43.");
  }
  const artifactPath = join(root, "contracts", "out", "LaunchpadFactoryV43.sol", "LaunchpadFactoryV43.json");
  const artifact = JSON.parse(await readFile(artifactPath, "utf8")) as {
    abi: unknown[];
    bytecode: { object: string };
  };
  const object = artifact.bytecode?.object;
  if (!object) throw new Error("Forge produced no LaunchpadFactoryV43 bytecode.");
...
async function assertLocalChain() {
  ...
  if (accounts.length < 6) throw new Error("The V43 demo requires at least six unlocked Anvil accounts.");
...
async function deploy() {
  ...
  if (!skipDemo) {
    const metadataHash = keccak256("perphood-v43-demo:HOOD") as Hex;
    const launch = await sendTransaction({
      from: creator,
      to: factoryAddress,
      data: encodeCreateMarket("PerpHood Unified", "HOOD", metadataHash, 45_000n * 10n ** 18n),
    ...
    if (!marketAddress || !tokenAddress) throw new Error("V43 MarketCreated event could not be decoded.");
    ...
    const spotBuy = await sendTransaction({
      from: spotTrader,
      to: marketAddress,
      data: functionSelector("buy()") as Hex,
      value: 750_000_000_000_000_000n,
    });
    manifest.demoSpotBuyTransactionHash = spotBuy.hash;

    const longCollateral = 40_000_000_000_000_000n;
    const longLeverage = 3n;
    const longNotional = longCollateral * longLeverage;
    const longFee = longNotional * 30n / 10_000n;
    const longOpen = await sendTransaction({
      from: longTrader,
      to: marketAddress,
      data: encodeStaticCall("openLong(uint16,uint16,uint256)", [longLeverage, 200n, longCollateral]),
      value: longCollateral + longFee,
    });
    manifest.demoLongTransactionHash = longOpen.hash;

    const shortCollateral = 20_000_000_000_000_000n;
    const shortLeverage = 2n;
    const shortNotional = shortCollateral * shortLeverage;
    const shortFee = shortNotional * 30n / 10_000n;
    const shortOpen = await sendTransaction({
      from: shortTrader,
      to: marketAddress,
      data: encodeStaticCall("openShort(uint16,uint16,uint256)", [shortLeverage, 200n, shortCollateral]),
      value: shortCollateral + shortFee,
    });
  ...
  console.log(`NEXT_PUBLIC_V43_LAUNCHPAD_FACTORY_ADDRESS=${manifest.factoryAddress}`);
  console.log(`NEXT_PUBLIC_LOCAL_CHAIN_RPC=${rpcUrl}`);
  if (manifest.demoMarketAddress) console.log(`NEXT_PUBLIC_V43_DEMO_MARKET_ADDRESS=${manifest.demoMarketAddress}`);
  if (manifest.demoTokenAddress) console.log(`NEXT_PUBLIC_V43_DEMO_TOKEN_ADDRESS=${manifest.demoTokenAddress}`);
}
...
else throw new Error(`Unknown V43 local-chain command: ${command}`);
