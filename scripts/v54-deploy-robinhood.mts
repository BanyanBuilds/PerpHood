...
const network = networkKey === "mainnet"
  ? { name: "Robinhood Chain Mainnet", chainId: 4_663, rpc: process.env.ROBINHOOD_MAINNET_RPC_URL ?? "https://rpc.mainnet.chain.robinhood.com", explorer: "https://robinhoodchain.blockscout.com", envName: "NEXT_PUBLIC_V54_MAINNET_FACTORY_ADDRESS" }
  : { name: "Robinhood Chain Testnet", chainId: 46_630, rpc: process.env.ROBINHOOD_TESTNET_RPC_URL ?? "https://rpc.testnet.chain.robinhood.com", explorer: "https://explorer.testnet.chain.robinhood.com", envName: "NEXT_PUBLIC_V54_TESTNET_FACTORY_ADDRESS" };

if (networkKey === "mainnet" && process.env.V54_ALLOW_MAINNET_DEPLOY !== "true") {
  throw new Error("Mainnet deployment is locked. Set V54_ALLOW_MAINNET_DEPLOY=true only after testnet launch verification is complete.");
}

const privateKey = process.env.V54_DEPLOYER_PRIVATE_KEY;
if (!privateKey || !/^0x[0-9a-fA-F]{64}$/.test(privateKey)) {
  throw new Error("Set V54_DEPLOYER_PRIVATE_KEY to a dedicated deployment-wallet key. Never commit it or add it to NEXT_PUBLIC variables.");
}
...
const args = [
  "create",
  "contracts/src/PerpHoodLaunchFactoryV54.sol:PerpHoodLaunchFactoryV54",
  "--rpc-url", network.rpc,
...
if (process.env.V54_VERIFY_CONTRACT === "true") {
...
const manifest = {
  ...
  verified: process.env.V54_VERIFY_CONTRACT === "true",
};
writeFileSync(resolve("deployments", `v54-${networkKey}.json`), `${JSON.stringify(manifest, null, 2)}\n`);

console.log(`\nV54 factory deployed: ${deployedAddress}`);
